define :pg_database, :action => :create do
  ...
  when :create
    ...
    createfunc.push "-W -f #{node["postgres"]["cache_path"]}/pgpool-recovery.sql #{params[:name]}"
    ...
    createfunc.push "--host #{defaults[:host]}" if defaults[:host]
    createfunc.push "--port #{defaults[:port]}" if defaults[:port]
    createfunc.push "-W -f #{node["postgres"]["cache_path"]}/pgpool-regclass.sql #{params[:name]}"
    createfunc = createfunc.join ' '
